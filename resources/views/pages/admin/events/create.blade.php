            ...
            <form action="{{ route('admin.events.store') }}" method="POST">
                ...
                <div class="row mb-6">
                    ...
                    <div class="col-lg-8">
                        <input type="text" name="title" class="form-control form-control-solid @error('title') is-invalid @enderror" 
                               value="{{ old('title') }}" placeholder="Event-Titel eingeben" required>
                        @error('title')
                ...
                <div class="row mb-6">
                    ...
                    <div class="col-lg-8">
                        <textarea name="description" class="form-control form-control-solid @error('description') is-invalid @enderror" 
                                  rows="3" placeholder="Event-Beschreibung (optional)">{{ old('description') }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row mb-6">
                    ...
                    <div class="col-lg-8">
                        <input type="text" name="category" class="form-control form-control-solid @error('category') is-invalid @enderror" 
                               value="{{ old('category') }}" placeholder="z.B. Kartenlegung, Meditation, etc." required>
                        @error('category')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row mb-6">
                    ...
                    <div class="col-lg-8">
                        <select name="category_color" class="form-select form-select-solid @error('category_color') is-invalid @enderror" required>
                            ...
                            <option value="bg-primary" {{ old('category_color') == 'bg-primary' ? 'selected' : '' }}>Primär (Blau)</option>
                            <option value="bg-success" {{ old('category_color') == 'bg-success' ? 'selected' : '' }}>Erfolg (Grün)</option>
                            <option value="bg-info" {{ old('category_color') == 'bg-info' ? 'selected' : '' }}>Info (Hellblau)</option>
                            <option value="bg-warning" {{ old('category_color') == 'bg-warning' ? 'selected' : '' }}>Warnung (Gelb)</option>
                            <option value="bg-danger" {{ old('category_color') == 'bg-danger' ? 'selected' : '' }}>Gefahr (Rot)</option>
                            <option value="bg-secondary" {{ old('category_color') == 'bg-secondary' ? 'selected' : '' }}>Sekundär (Grau)</option>
                        </select>
                ...
                <div class="card-footer d-flex justify-content-end py-6 px-9">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <span class="indicator-label">Event erstellen</span>
                    </button>
                </div>
            </form>
        </div>
